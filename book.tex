...
O \cref{thm:backdoor_est_1} determina que, 
se $\Z$ satisfaz o critério backdoor, então
...
\begin{theorem}
 ...
 medir o efeito causal de $X$ em $Y$,
 $\E[|\mu(x,\Z)|] < \infty$, e
 $\E[(\hat{\mu}(x,\Z)-\mu(x,\Z)^2] = o(1)$, então
...
motivada pelo \cref{thm:backdoor_est_2}.
...
\begin{theorem}
 \label{thm:conv_ipw}
 Se $\E[|\hf(x|\Z)-f(x|\Z)|] = o(1)$, e
 existe $M > 0$ tal que
 $\sup_\z \E[|Y| \I(X=x)|\Z=\z] < M$, e
 existe $\delta > 0$ tal que
 $\inf_\z \hf(x|\z) > \epsilon$ e
 $\inf_\z f(x|\z) > \epsilon$, então
...
\begin{proof}[Prova do \cref{thm:conv_ajuste}]
 ...
 \begin{align*}
  \P\left(\bigg|\hdoy - \frac{\sum_{i=1}^n \mu(x,\Z_i)}{n}\bigg|
  > \epsilon \right)
  &= \P\left(\bigg|\frac{\sum_{i=1}^n 
  (\hat{\mu}(x,\Z_i)-\mu(x,\Z_i))}{n}\bigg|
  > \epsilon \right) \\
  &= \P\left(\left(\sum_{i=1}^n 
  (\hat{\mu}(x,\Z_i)-\mu(x,\Z_i))\right)^2
  > n^2 \epsilon^2 \right) \\
  &\leq \frac{\E\left[\left(\sum_{i=1}^n 
  (\hat{\mu}(x,\Z_i)-\mu(x,\Z_i))\right)^2
  \right]}
  {n^2 \epsilon^2}
  & \text{Markov} \\
  &\leq \frac{n^2 \E[(\hat{\mu}(x,\Z_1)-\mu(x,\Z_1))^2]}
  {n^2 \epsilon^2}
  & \text{Cauchy-Schwarz} \\
  &= o(1)
 \end{align*}
 A última conclusão decorre da suposição de que
 $\E[(\hat{\mu}(x,\Z)-\mu(x,\Z))^2] = o(1)$.
\end{proof}
...
\begin{proof}[Prova do \cref{thm:conv_ipw}]
 ...
 é suficiente provar que
 \begin{align*}
  n^{-1} \sum_{i=1}^n \frac{Y_i \I(X_i = x)}{\hf(x|\Z_i)} 
  - n^{-1} \sum_{i=1}^n \frac{Y_i \I(X_i = x)}{f(x|\Z_i)}
  &\convp 0
 \end{align*}
 \begin{align*}
  &\P\left(\bigg| n^{-1} \sum_{i=1}^n \frac{Y_i \I(X_i = x)}{\hf(x|\Z_i)}
  - n^{-1} \sum_{i=1}^n \frac{Y_i \I(X_i = x)}{f(x|\Z_i)}
  \bigg| > \epsilon \right) \\
  =& \P\left(\bigg| \sum_{i=1}^n \frac{Y_i \I(X_i = x)
  (\hf(x|\Z_i)-f(x|\Z_i))}{\hf(x|\Z_i)f(x|\Z_i)} \bigg| 
  > n\epsilon \right) \\
  \leq& \P\left(\bigg| \sum_{i=1}^n Y_i \I(X_i = x)
  (\hf(x|\Z_i)-f(x|\Z_i)) \bigg| 
  > n\delta^2\epsilon \right) 
  & \sup_\z \hf(x|\z) f(x|\z) > \delta^2 \\
  \leq& \frac{\E\left[\bigg| \sum_{i=1}^n Y_i \I(X_i = x)
  (\hf(x|\Z_i)-f(x|\Z_i)) \bigg|\right]} 
  {n\delta^2\epsilon} 
  & \text{Markov} \\
  \leq& \frac{n\E\left[|Y \I(X = x)(\hf(x|\Z)-f(x|\Z))|\right]} 
  {n\delta^2\epsilon} 
  & \text{i.i.d.} \\
  \leq& \frac{\E\left[|\hf(x|\Z)-f(x|\Z)|\E[|Y| \I(X = x)|\Z]\right]} 
  {\delta^2\epsilon} \\
  \leq& \frac{M\E\left[|\hf(x|\Z)-f(x|\Z)|\right]} 
  {\delta^2\epsilon} 
  & \sup_\z \E[|Y| \I(X = x)|\Z=\z] < M \\
  =& o(1)
 \end{align*}
\end{proof}
