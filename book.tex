...
\hlkwd{adjustmentSets}\hlstd{(grafo,} \hlstr{"X"}\hlstd{,} \hlstr{"Y"}\hlstd{,}
               \hlkwc{type} \hlstd{=} \hlstr{"all"}\hlstd{)}
\end{alltt}
\begin{verbatim}
## { A, C }
## { B, C }
...
nos \cref{thm:backdoor_est_1,thm:backdoor_est_2}.
...
\cref{thm:backdoor_est_1,thm:backdoor_est_2} podem
...
\subsubsection{Exemplos de estimação causal com o critério backdoor}

Nesta seção veremos exemplos de 
como estimar o efeito causal de $X$ em $Y$
utilizando o critério backdoor.

\begin{example}[Fórmula do ajuste]
 \label{ex:backdoor_est_1}
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{figure}[t]
...
\hlstd{X} \hlkwb{<-} \hlkwd{as.numeric}\hlstd{((A} \hlopt{+} \hlstd{B)} \hlopt{>} \hlnum{0}\hlstd{)}
...
\hlstd{\{}
  \hlstd{var_backdoor} \hlkwb{<-} \hlstd{dagitty}\hlopt{::}\hlkwd{adjustmentSets}\hlstd{(grafo, causa, efeito)[[}\hlnum{1}\hlstd{]]}
...
\hlstd{mu_chapeu} \hlkwb{<-} \hlkwd{lm}\hlstd{(fm,} \hlkwc{data} \hlstd{= data)}
\hlstd{ace_est} \hlkwb{=} \hlkwd{est_do_x_lm}\hlstd{(data, mu_chapeu,} \hlstr{"X"}\hlstd{,} \hlnum{1}\hlstd{)} \hlopt{-}
  \hlkwd{est_do_x_lm}\hlstd{(data, mu_chapeu,} \hlstr{"X"}\hlstd{,} \hlnum{0}\hlstd{)}
\hlkwd{round}\hlstd{(ace_est)}
\end{alltt}
\begin{verbatim}
## [1] 2
\end{verbatim}
...
\hlstd{mu_chapeu} \hlkwb{<-} \hlkwd{xgboost}\hlstd{(}
  \hlkwc{data} \hlstd{= data} \hlopt{%>%} \hlstd{dplyr}\hlopt{::}\hlkwd{select}\hlstd{(A, B, X)} \hlopt{%>%} \hlkwd{as.matrix}\hlstd{(),}
  \hlkwc{label} \hlstd{= data} \hlopt{%>%} \hlstd{dplyr}\hlopt{::}\hlkwd{select}\hlstd{(Y)} \hlopt{%>%} \hlkwd{as.matrix}\hlstd{(),}
...
\hlstd{\{}
  \hlstd{data} \hlopt{%>%}
    \hlstd{dplyr}\hlopt{::}\hlkwd{mutate}\hlstd{(\{\{causa\}\}} \hlkwb{:=} \hlstd{x)} \hlopt{%>%}
    \hlstd{dplyr}\hlopt{::}\hlkwd{select}\hlstd{(A, B, X)} \hlopt{%>%}
...
\end{knitrout}
 ...
 \end{align*}
 Portanto, as estimativas obtidas do ACE
 estão adequadas.
\end{example}
...
\section{Relativas à Seção \ref{sec:backdoor}}
...
\begin{proof}[Prova do \cref{thm:conv_ajuste}]
 ...
 \begin{align*}
  ...
  &\convp \E[\mu(x,\Z)]
 \end{align*}
 Portanto, pelo \cref{thm:backdoor_est_1},
...
\begin{proof}[Prova do \cref{thm:conv_ipw}]
 ...
 pelo \cref{thm:backdoor_est_2} temos que
