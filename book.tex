...
\textbf{Agradecimentos}: Caio S. B. Mota, Ciro A. Higashi, Stefan Z. Gonçalves, Vitor Mello
...
Considere que observamos em $700$ pacientes $3$ variáveis: 
...
\begin{exercise}[{\citet{Glymour2016}[p.29]}]
 ...
 \begin{exercise}
  Verifique que a demonstração do \cref{thm:ace_linear} não utilizou a suposição de que os resíduos do modelo linear Gaussiano são normais. Qual é o modelo mais geral que você consegue imaginar em que o \cref{thm:ace_linear} ainda vale?
 \end{exercise}
 
 \begin{exercise}
  Considere que $\sV = \{X,Y,Z\}$ e o grafo causal é tal que
  $Pa(Y) = \{X,Z\}$ e $Pa(Z) = \{X\}$. Também,
  $\E[Y|Pa(Y)] = \beta_{1} \cdot Z \cdot X$, e
  $\E[Z|Pa(Z)] = \beta_2 \cdot X$. Determine o efeito causal
  de $X$ em $Y$. É possível obter este resultado por
  meio do \cref{thm:ace_linear}?
 \end{exercise}
...
\begin{definition}
 ...
 \begin{align*}
  \int_{\mathcal{Z}} f(y|do(x),\z)f(\z|do(x)) 
  &= \int_{\mathcal{Z}} f(y|x,\z)f(\z) d\z
 \end{align*}
 Em particular,
 \begin{align*}
  f(y|do(x)) &= \int f(y|do(x),\z)f(\z|do(x)) \\
  &= \int f(y|x,\z)f(\z) d\z
 \end{align*}
\end{definition}
...
\begin{theorem}
 ...
 \begin{enumerate}
  \item $\hmu$ é invariante a permutações (\cref{def:perm}),
  \item $\E[|\mu(x,\Z_1)|] < \infty$,
  \item $\E[|\hmu(x,\Z_1)-\mu(x,\Z_1)|] = o(1)$, e
 \end{enumerate}
 então $\hdoy \convp \E[Y|do(X=x)]$.
\end{theorem}
...
\begin{theorem}
 \label{thm:conv_ipw}
 Se $\Z$ satisfaz a \cref{def:conf_control} e, também
 \begin{enumerate}
  \item $\hf$ é invariante a permutações (\cref{def:perm}),
  \item $\E[|\hf(x|\Z_1)-f(x|\Z_1)|] = o(1)$,
  \item existe $M > 0$ tal que $\sup_\z \E[|Y| \I(X=x)|\Z=\z] < M$, e
  \item existe $\delta > 0$ tal que $\inf_z \min\{f(x|\Z_1), \hf(x|\Z_1)\} > \delta$,
 \end{enumerate}
 então $\hdoyb \convp \E[Y|do(X=x)]$.
\end{theorem}
...
\begin{definition}
 ...
 \begin{enumerate}
  ...
  e $i$ tal que $C_i = W$.
  \item $\emptyset$ satisfaz o critério backdoor 
  (\cref{def:backdoor}) para medir o efeito causal de 
  $X$ em $W$, para cada $W \in \W$.
  \item $X$ satisfaz o critério backdoor 
  (\cref{def:backdoor}) para 
  medir o efeito causal de $W$ em $Y$, para cada $W \in \W$.
 \end{enumerate}
\end{definition}
...
\begin{definition}[Ignorabilidade]
 ...
 medir o efeito causal em $Y$ se
 $Y_x \ind{f^*} X$, 
 para todo $f$ compatível com o grafo causal e
 respectiva $f^*$.
\end{definition}
...
\begin{lemma}
 ...
 \begin{enumerate}
  ...
  medir o efeito causal de $X$ em $Y$,
  \item $(Anc(X)-\{X\}) \cap (Anc(Y)-\{Y\}) = \emptyset$, isto é,
...
O critério backdoor é uma condição 
mais forte que a ignorabilidade condicional.
Isto acontece pois o segundo critério não exige que
$X \notin Anc(\Z)$, conforme 
formalizado no \cref{theorem:cignore_backdoor}.

\begin{theorem}[Relação entre critério backdoor e ignorabilidade condicional]
 \label{theorem:cignore_backdoor}
 \begin{enumerate}[label=(\roman*)]
  \item Se $\Z$ satisfaz o critério backdoor para 
  medir o efeito causal de $X$ em $Y$, então
  $X$ é condicionalmente ignorável para 
  medir o efeito causal em $Y$ dado $\Z$.
  \item Suponha que $X \notin Anc(\Z)$.
...
Ainda que não haja uma resposta única para esta pergunta,
...
\begin{example}
 ...
 o erro e a amputação: $\P(Y_{X=0}=1|X=1,Y=1)$.
 \begin{align*}
  \P(Y_{X=0}=1|X=1,Y=0)
...
tanto da distribuição de $U_Y$ quanto da função $g_Y$.
...
\begin{example}
 ...
 \end{align*}
 Neste caso, $g_Y$ induz a mesma distribuição 
...
Esta outra interpretação pode ser formalizada 
tomando $\mathcal{Q} = \{Q^*: \E[U(\sV,Q^*)] = \E[U(\sV_{X=0},0)]\}$
e definindo:
\begin{align*}
 Q = \arg\min_{Q^* \in \mathcal{Q}}\E[(U(\sV,Q^*(E)) - U(\sV_{X=0},0))^2]
\end{align*}
...
considerarmos o grafo causal sem arestas,
``$X \hspace{3 mm} Y$'', 
...
\begin{definition}
 ...
 Os DAGs $\sG$ e $\sG^*$ têm o mesmo padrão se:
 \begin{enumerate}
...
\begin{proof}[Prova do \cref{thm:backdoor_ajuste}]
 \begin{align}
  ...
  \int_{\mathcal{Z}} \E[g(Y)|do(X=x),\z] f(\z|do(X=x)) d\z
  &= \int_{\mathcal{Z}} \int g(y) f(y|do(x),\z) dy f(\z|do(X=x)) d\z \nonumber \\
  &= \int_{\mathcal{Z}} g(y) \int f(y|do(x),\z) f(\z|do(X=x)) d\z dy \nonumber \\
  &= \int_{\mathcal{Z}} g(y) \int f(y|x,\z) f(\z) d\z dy
  & \text{\cref{def:conf_control}} \nonumber \\
  &= \int_{\mathcal{Z}} \int g(y)f(y|x,\z) dy f(\z) d\z \nonumber \\
  &= \int_{\mathcal{Z}} \E[g(Y)|X=x,\z] f(\z) d\z \\
  \E[g(Y)|do(X=x)]
  &= \E[\E[g(Y)|do(X=x),\Z]] \nonumber \\
  &= \E[\E[g(Y)|X=x,\Z]]
...
\begin{proof}[Prova do \cref{thm:backdoor_ipw}]
 \begin{align*}
  \int_{\mathcal{Z}} \E[g(Y)|do(x),\z] f(\z|do(x)) d\z 
  &= \int_{\mathcal{Z}} \int g(y) f(y|do(x),\z) f(\z|do(x)) dy d\z \\
  &= \int g(y) \int_{\mathcal{Z}} f(y|do(x),\z) f(\z|do(x)) d\z dy \\
  &= \int g(y) \int_{\mathcal{Z}} f(y|x,\z) f(\z) d\z dy 
  & \text{\cref{def:conf_control}} \\
  &= \int_{\mathcal{Z}} f(\z) \int g(y) f(y|x,\z)  dy d\z \\
  &= \int_{\mathcal{Z}} f(\z) \int \frac{g(y) f(y,x|\z)}{f(x|\z)} dy d\z \\
  &= \int_{\mathcal{Z}} f(\z) \int \frac{g(y) \I(x_* = x) f(y,x_*|\z)}{f(x|\z)} d(x_*, y) d\z \\
  &= \int_{\mathcal{Z}} f(\z) \E\left[\frac{g(Y)\I(X=x)}{f(x|\z)}|\z\right] d\z \\
  &= \int_{\mathcal{Z}} \frac{\E[g(Y)\I(X=x)|\z]}{f(x|\z)} f(\z) d\z
 \end{align*}
 \begin{align*}
  \E[Y|do(x)]
  &= \E[\E[Y|do(X),\Z]] \\
...
\section{\text{\cref{sec:frontdoor,sec:do_calculus} (\nameref{sec:frontdoor})}}

\subsection{\text{\cref{thm:do_calculus}}}
...
\subsection{\text{\cref{thm:frontdoor}}}
...
\subsection{\text{\cref{thm:frontdoor_ipw}}}
...
\begin{proof}[Prova do \cref{lemma:ignore_backdoor}]
 ...
 ($3 \rightarrow 1$)
 Esta demonstração decorre do \cref{theorem:cignore_backdoor},
 provado a seguir, tomando $\Z = \emptyset$.
\end{proof}

\begin{proof}[Prova do \cref{theorem:cignore_backdoor}]
 (i) Suponha que $\Z$ satisfaz o critério backdoor 
 para medir o efeito causal de $X$ em $Y$.
 \begin{align*}
  \P(Y_x | Z)
  &= \P(Y_x | Z_x) & \text{\cref{ex:pom_anc}} \\
  &= \P(Y|Z, do(X=x)) &\text{\cref{lemma:po_do}} \\
  &= \P(Y|Z, X=x) & \text{\cref{thm:backdoor}} \\
  &= \P(Y_x|Z, X=x) & \text{\cref{lemma:obs_to_potential}}
 \end{align*}
 Assim, $Y_x \ind^{f^*} X$.
 
 (ii) Suponha que $\Z$ não satisfaz 
