...
\caption[Grafo que representa as relações causais entre S (Sexo), T (Tratamento), e C (Cura)]{Grafo que representa as relações causais entre S (Sexo), T (Tratamento), e C (Cura).}\label{fig:simpson_sexo}
\end{figure}

\end{knitrout}

Para iniciar esta discussão, considere
as variáveis $S$ (Sexo), $T$ (Tratamento), 
e $C$ (Cura), discutidas no \cref{cap:intro}.
Podemos considerar que $S$ é uma causa
tanto de $T$ quanto de $C$ e que
$T$ é uma causa de $C$. Assim,
...
calculando diretamente $f(c|t)$:
\begin{align}
 \label{eq:prob_causa_1}
 f(c|t)
 &= \sum_s f(s,c|t) \nonumber \\
 &= \sum_s \frac{f(s,c,t)}{f(t)} \nonumber \\
 &= \sum_s \frac{f(s,t)f(c|s,t)}{f(t)} \nonumber \\
 &= \sum_s f(s|t)f(c|s,t)
\end{align}
Notamos que $f(c|t)$ é a média das
probabilidades de cura em 
cada sexo, $f(c|s,t)$,
ponderadas pela distribuição do sexo
após aprender o tratamento do indivíduo, $f(s|t)$.
...
em que apenas estamos observando a população,


Na última linha, a informação obtida do tratamento é
usada tanto em $f(s|t)$ quanto em $f(c|s,t)$.
Ainda que 
